...
class BranchAndBoundSchedulerGPU : public Scheduler {
 public:
   virtual auto schedule_impl(
      ...
      const std::size_t sequential_makespan = device_sequential_makespan(
           sequence);
      ...
      for (auto x : sequence.ops) {
         x.is_scheduled = false;
      }
      ...
      const std::size_t lower_bound = device_critical_path(working_copy);
      ...
      auto place_initial_accumulations =
           [&](const std::vector<std::size_t>& early_accs) -> bool {
         // Return if time's up
         if (!remaining_time()) {
            return false;
         }
         for (size_t t = 0; t < std::min(early_accs.size(), usable_threads);
              t++) {
            working_copy.ops[early_accs[t]].is_scheduled = true;
            working_copy.ops[early_accs[t]].start_time = 0;
            working_copy.ops[early_accs[t]].thread = t;

            thread_loads[t] = sequence.ops[early_accs[t]].fma;
      ...
      auto nonrecursive_schedule_op = [&]() -> bool {
         ...
         for (std::size_t op_idx_temp = 0; op_idx_temp < sequence.length;
              ++op_idx_temp) {
         ...
         std::vector<Mode> modes_in_sequence = {
              Mode::ADJOINT, Mode::TANGENT, Mode::ADJOINT, Mode::ADJOINT,
              Mode::TANGENT, Mode::ADJOINT, Mode::TANGENT, Mode::NONE,
              Mode::NONE,    Mode::NONE,    Mode::NONE};
         std::vector<Action> actions_in_sequence = {
              Action::ACCUMULATION,   Action::ACCUMULATION,
              Action::ACCUMULATION,   Action::ACCUMULATION,
              Action::ACCUMULATION,   Action::ELIMINATION,
              Action::ELIMINATION,    Action::MULTIPLICATION,
              Action::MULTIPLICATION, Action::MULTIPLICATION,
              Action::MULTIPLICATION};

         bool optimal_tree = false;  // set to true for debugging the loop with
                                     // setup in text
         for (size_t i = 0; i < sequence.length; i++) {
            if (sequence.ops[i].mode != modes_in_sequence[i] ||
                sequence.ops[i].action != actions_in_sequence[i]) {
               optimal_tree = false;
               break;
            }
         }
         ...
         if (sequence.length == 11) {
            for (size_t i = 0; i < sequence.length; i++) {
               ////std::println("{} , {}", sequence[i].mode,
               /// sequence[i].action);  enable for finding the current
               /// tree/sequence
            }
         }
         ...
         std::println(
              "{}--------------------------------------------------------------"
              "--------------{}",
              sequence.length, usable_threads);

         while (remaining_time() > 0.0) {

            std::vector<size_t> target {
                 8722552u, 11946528u, 7683592u, 9603792u};

            if (thread_loads == target) {
               optimal_tree = false;
               ////std::println("Double the threadloads");  use for debugging
               /// the loop with setup in text
            }
            ...
            if (op_idx >= sequence.length) {
               std::println(
                    "opidx {} exceeded length {}", op_idx, sequence.length);
            ...
            if (working_copy.ops[op_idx].is_scheduled or
               ...
               if (debug8count < 140 && optimal_tree) {
                  std::println(
                       "nextop/opidx increase unregistered. opidx {} is "
                       "scheduled {} schedulable {}",
                       op_idx - 1, working_copy.ops[op_idx - 1].is_scheduled,
                       !is_schedulable(working_copy, op_idx - 1));
               }
               if (op_idx >= sequence.length) {
                  revert_op_idx = true;
                  skip_changes = true;
            ...
            if (thread_idx >= usable_threads) {
               if (debug8count < 140) {
                  ////std::println("threadidx {} exceeded usable threads {}",
                  /// thread_idx, usable_threads);
               }
            ...
            if (debug8count < 140 && optimal_tree) {
               debug8count++;
               std::println(
                    "Depth: {}, Op idx: {}, Thread idx: {}, Best makespan: {}, "
                    "Current makespan: {}",
                    depth, op_idx, thread_idx, best_makespan, makespan);
            }
            ...
            if (!skip_changes) {
               ...
               const std::size_t start_time = std::max(
                    thread_loads[thread_idx],
                    earliest_start(working_copy, op_idx));
               ////std::println("threadloads: {},threadloads(i): {},
               /// earlieststart: {}, starttime: {}",thread_loads,
               /// thread_loads[thread_idx],working_copy.earliest_start(op_idx),start_time);
               working_copy.ops[op_idx].start_time = start_time;
               idling_time += (start_time - thread_loads[thread_idx]);
               thread_loads[thread_idx] = start_time + sequence.ops[op_idx].fma;
               makespan = std::max(makespan, thread_loads[thread_idx]);
               if (debug8count < 140 && optimal_tree) {
                  std::println("{}", working_copy.ops[op_idx]);
               }
            }
            ...
            if (depth >= sequence.length - 1) {
               if (makespan < best_makespan) {
                  ...
                  // std::println("Finished a schedule with makespan {} with
                  // current best {}", makespan, best_makespan);
                  best_makespan = makespan;
                  for (size_t i = 0; i < sequence.length; ++i) {
                     sequence.ops[i].thread = working_copy.ops[i].thread;
                     sequence.ops[i].start_time =
                          working_copy.ops[i].start_time;
            ...
            if (!skip_changes && depth < sequence.length - 1) {
               ...
               if (std::max(lb, makespan) < best_makespan) {
                  ...
                  current_layer.start_time_op =
                       working_copy.ops[op_idx].start_time;
                  ...
                  if (debug8count < 140 && optimal_tree) {
                     std::println(
                          "work/critpath/makespan too high {} / {} / {}, "
                          "revert thread",
                          ((idling_time + sequential_makespan) /
                           usable_threads),
                          device_critical_path(working_copy), makespan);
                     if (device_critical_path(working_copy) >
                         device_sequential_makespan(working_copy)) {
            ...
            if (debug8count < 140 && optimal_tree) {
               for (size_t i = 0; i < sequence.length; i++) {
                  std::println(
                       "op {} start time {} and scheduled {} on t: {}", i,
                       working_copy.ops[i].start_time,
                       working_copy.ops[i].is_scheduled,
                       working_copy.ops[i].thread);
               }
            ...
            if (revert_thread_idx) {
               ...
               if (op_idx < sequence.length) {
                  ...
                  std::println(
                       "revert threadidx {} EXCEEDED length {}", op_idx,
                       sequence.length);
               }
               ...
               if (debug8count < 140 && optimal_tree) {
                  std::println(
                       "REVERT THREAD; threadidx {} nothing else as "
                       "revertothers {} de {}",
                       thread_idx, revert_op_idx, revert_depth);
               }
            }
            ...
            if (revert_op_idx) {
               ...
               if (op_idx < sequence.length) {
                  ...
                  if (debug8count < 140 && optimal_tree) {
                     std::println(
                          "revert opidx {} EXCEEDED length {}", op_idx,
                          sequence.length);
                  }
               }
               ...
               if (debug8count < 140 && optimal_tree) {
                  std::println(
                       "REVERT OP; opidx {} in state {}, revert depth set {}",
                       op_idx, previous_state.op_idx, revert_depth);
               }
            }
            ...
            if (revert_depth) {
               ...
               depth--;
               working_copy.ops[progress_stack.top().op_idx].is_scheduled =
                    false;
               working_copy.ops[progress_stack.top().op_idx].start_time = 0;
               progress_stack.pop();
               Layer previous_state = progress_stack.top();
               ...
               if (debug8count < 140 && optimal_tree) {
                  std::println(
                       "REVERT DEPTH; depth:{} opidx: {}", depth, op_idx);
                  std::println(
                       "mkspn {}, idltme {}, starttime {}, threadloads {}",
                       makespan, idling_time,
                       working_copy.ops[op_idx].start_time, thread_loads);
               }
            }
         }
         return false;
